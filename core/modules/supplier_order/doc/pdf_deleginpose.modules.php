...
/**
 *	\file       htdocs/core/modules/supplier_order/pdf/pdf_deleginpose.modules.php
 *	\ingroup    fournisseur
 ...
 */
class pdf_deleginpose extends ModelePDFSuppliersOrders
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Délégation LMDB";
		$this->description = $langs->trans('SuppliersDelegationModelLMDB');
	...
	function write_file($object,$outputlangs='',$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->fournisseur->dir_output.'/commande')
		{
			...
			else
			{
				...
				$file = $dir . "/" . $objectref . " - Délégation de Paiement INPOSE.pdf";
				if (! empty($conf->global->SUPPLIER_REF_IN_NAME)) $file = $dir . "/" . $objectref . " - Délégation de Paiement INPOSE.pdf";
			}
			...
			if (file_exists($dir))
			{
				...
				if (empty($conf->global->MAIN_DISABLE_FPDI) && ! empty($conf->global->MAIN_ADD_PDF_BACKGROUND_DELEGINPOSEx))
                {
                    $pagecount = $pdf->setSourceFile($conf->mycompany->dir_output.'/'.$conf->global->MAIN_ADD_PDF_BACKGROUND_DELEGINPOSEx);
				...
				include DOL_DOCUMENT_ROOT.'/core/modules/supplier_order/pdf/deleginpose/deleginpose.php';
