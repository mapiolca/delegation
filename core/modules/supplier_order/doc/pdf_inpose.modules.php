 ...
 */
class pdf_INPOSE extends ModelePDFSuppliersOrders
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Commande Fournisseur INPOSE";
		$this->description = $langs->trans('SuppliersCommandModelINPOSE');
	...
	function write_file($object,$outputlangs='',$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->fournisseur->dir_output.'/commande')
		{
			...
			if (file_exists($dir))
			{
				...
				{
		                $pagecount = $pdf->setSourceFile(DOL_DOCUMENT_ROOT.'/custom/delegation/core/modules/supplier_invoice/pdf/lcr/fontlcr.pdf');
		                $tplidx = $pdf->importPage(1);
...

	{
		...
		include DOL_DOCUMENT_ROOT.'/core/modules/supplier_invoice/pdf/lcr/lcrtext.php';
		...
		// Remplissage de la LCR
			...
			//Ordre
				$pdf->writeHTMLCell(190,4, 110, 200,$object->thirdparty->name,0,1);
				$pdf->writeHTMLCell(50,10, 110, 205,$object->thirdparty->getFullAddress(),0,1);
			...
			//Montant TTC Mois pour contrôle
				$pdf->writeHTMLCell(190,4, 15, 225, dol_htmlentitiesbr(price($object->total_ttc)),0,1);

			//Date de Création
				$pdf->writeHTMLCell(190,4, 45, 225, dol_htmlentitiesbr(dol_print_date($object->date,"day",false,$outputlangs)),0,1);

			//Date d'Échéance
				$pdf->writeHTMLCell(190,4, 75, 225, dol_htmlentitiesbr(dol_print_date($object->array_options['options_date_lcr'],"day",false,$outputlangs,true)),0,1);

			//Montant
				$pdf->writeHTMLCell(190,4, 175, 225, dol_htmlentitiesbr(price($object->total_ttc)),0,1);

			//Nom du Tiré
				$pdf->writeHTMLCell(190,4, 100, 250, $carac_client_name,0,1);

			//Adresse du Tiré
				$pdf->writeHTMLCell(40,10, 100, 255, $mysoc->getFullAddress(),0,1);
			...
			////Banque
				...
				$pdf->writeHTMLCell(60,10, 140, 245, $outputlangs->convToOutputCharset($account->bank),0,1);

				$pdf->writeHTMLCell(60,10, 10, 245, $outputlangs->convToOutputCharset($account->code_banque),0,1);
				$pdf->writeHTMLCell(60,10, 29, 245, $outputlangs->convToOutputCharset($account->code_guichet),0,1);
				$pdf->writeHTMLCell(60,10, 45, 245, $outputlangs->convToOutputCharset($account->number),0,1);
				$pdf->writeHTMLCell(60,10, 72, 245, $outputlangs->convToOutputCharset($account->cle_rib),0,1);
