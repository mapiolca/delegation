 ...
 */
class pdf_BLINPOSE extends ModelePDFSuppliersOrders
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Bon de Livraison LMDB";
		$this->description = $langs->trans('SuppliersDeliveryModelLMDB');
		...
		if ($conf->global->PRODUCT_USE_UNITS)
		{
			...
			$this->posxqty=153;
			$this->posxqty_livraison=116;
			$this->posxunit=170;
		} else {
			$this->posxqty=160;
			$this->posxqty_livraison=128;
		}
	...
	function write_file($object,$outputlangs='',$srctemplatepath='',$hidedetails=0,$hidedesc=0,$hideref=0)
	{
		...
		if ($conf->fournisseur->dir_output.'/commande')
		{
			...
			else
			{
				...
				$file = $dir . "/BL_" . $objectref . "_".date(dmy).".pdf";
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0 ; $i < $nblignes ; $i++)
				{
					...
					pdf_writelinedesc($pdf,$object,$i,$outputlangs,100,3,$curX,$curY,$hideref,$hidedesc,1);
					...
					// Quantité Livré
					$pdf->SetXY($this->posxqty_livraison, $curY);
					// Enough for 6 chars
					if($conf->global->PRODUCT_USE_UNITS)
					{
						$lineid = $object->lines[$i]->rowid;
						$pdf->MultiCell($this->posxunit-$this->posxqty_livraison-0.8, 4, $object->customfields->lines->$lineid->qty_livraison, 0, 'R');
					}
					else
					{

						$lineid = $object->lines[$i]->rowid;
						$pdf->MultiCell($this->posxdiscount-$this->posxqty_livraison-0.8, 4, $object->customfields->lines->$lineid->qty_livraison, 0, 'R');
					}
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0)
	{
		...
		$pdf->line($this->posxqty_livraison-1, $tab_top, $this->posxqty_livraison-1, $tab_top + $tab_height);
		if (empty($hidetop))
		{
			$pdf->SetXY($this->posxqty_livraison-1, $tab_top+1);
			if($conf->global->PRODUCT_USE_UNITS)
			{
				$pdf->MultiCell($this->posxunit-$this->posxqty_livraison-2,2, $outputlangs->transnoentities("Quantité en Livraison"),'','L');
			}
			else
			{
				$pdf->MultiCell($this->posxdiscount-$this->posxqty_livraison-2,2, $outputlangs->transnoentities("Quantité en Livraison"),'','L');
			}
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$title=$outputlangs->transnoentities("Bon de Livraison")." ".$outputlangs->convToOutputCharset($object->ref);
		...
		if (!empty($conf->global->SUPPLIER_ORDER_USE_HOUR_FOR_DELIVERY_DATE)) $usehourmin='dayhour';
			...
			$pdf->MultiCell(190, 3, $outputlangs->transnoentities("DateDeliveryPlanned")." : " . date(d)."/".date(m)."/".date(y), '', 'R');
