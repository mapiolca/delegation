 ...
 */
class pdf_DC42017_Commande extends ModelePDFCommandes
{
    var $db;
    var $name;
    var $description;
				...
				if ($conf->delegation->enabled)
				{
					...
					if ($id > 0)
					{
						...
						if ($dc4_result > 0)
						{
							...
							if ($object->element != 'commande')// || $object->type != 5)
							{
								$error = true;
...
*/
			...
			// Page 2
				...
				//Contenu
					...
					$pdf->writeHTMLCell(190,4, 15, 69, dol_htmlentitiesbr($outputlangs->convToOutputCharset($Societe)),0,1);

					$pdf->writeHTMLCell(190,4, 110, 69, dol_htmlentitiesbr($outputlangs->convToOutputCharset($SIREN_Societe)),0,1);

					$pdf->writeHTMLCell(190,4, 15, 96, dol_htmlentitiesbr($outputlangs->convToOutputCharset($Forme_Juridique_SousTraitant)),0,1);

					$pdf->writeHTMLCell(190,4, 15, 166, dol_htmlentitiesbr($outputlangs->convToOutputCharset($SousTraitant)),0,1);
					
					$pdf->writeHTMLCell(190,4, 110, 166, dol_htmlentitiesbr($outputlangs->convToOutputCharset($SIREN_SousTraitant)),0,1);

					$pdf->writeHTMLCell(190,4, 15, 194, dol_htmlentitiesbr($outputlangs->convToOutputCharset($Forme_Juridique_SousTraitant)),0,1);
			...
			// Page 3
				...
				//Banque
					...
					if (empty($object->mode_reglement_code) || $object->mode_reglement_code == 'VIR' || $object->mode_reglement_code == 'DEL')
					{

						if (! empty($object->array_options['options_lmdb_commande_account']))
						{
							$bankid=$object->array_options['options_lmdb_commande_account'];
							$account = new Account($this->db);
							$account->fetch($bankid);

						} else if ($conf->global->BANK_ASK_PAYMENT_BANK_DURING_ORDER) {
					...
					$posy=pdf_bank($pdf,$outputlangs,100,238,$account,0,$default_font_size);
