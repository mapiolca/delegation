 ...
 */
class pdf_csstbtps2014_Commande extends ModelePDFCommandes
{
    var $db;
    var $name;
    var $description;
...
//					$pdf->AddPage();
			...
			// Page 1
				...
				//include DOL_DOCUMENT_ROOT.'/custom/delegation/core/modules/supplier_order/pdf/contrat-sous-traitance-btp-simplifie-2014/contrat-sous-traitance-btp-simplifie-2014.php';
				...
				//Contenu
					...
					$pdf->writeHTMLCell(190,4, 71, 30.5, dol_htmlentitiesbr($outputlangs->convToOutputCharset($object->thirdparty->name)),0,1);
					...
					$pdf->writeHTMLCell(190,4, 71, 35, dol_htmlentitiesbr($outputlangs->convToOutputCharset($Forme_Juridique_SousTraitant)),0,1);

					$pdf->SetFont('','B',8); // fixe la police, le type ( 'B' pour gras, 'I' pour italique, '' pour normal,...)
					
					$pdf->writeHTMLCell(190,4, 140, 35, dol_htmlentitiesbr($outputlangs->convToOutputCharset(price($object->thirdparty->capital))),0,1);

					$pdf->writeHTMLCell(190,4, 71, 41, dol_htmlentitiesbr($outputlangs->convToOutputCharset($Adresse_sst)),0,1);

					$pdf->writeHTMLCell(190,4, 71, 45.5, dol_htmlentitiesbr($outputlangs->convToOutputCharset($SIRETCIFsst)),0,1);

					$pdf->writeHTMLCell(190,4, 71, 57.5, dol_htmlentitiesbr($outputlangs->convToOutputCharset($object->thirdparty->array_options['options_lmdb_representant'])),0,1);

					$pdf->writeHTMLCell(190,4, 71, 62.6, dol_htmlentitiesbr($outputlangs->convToOutputCharset($object->thirdparty->array_options['options_lmdb_qualite_representant'])),0,1);

					$pdf->writeHTMLCell(190,4, 71, 78, dol_htmlentitiesbr($outputlangs->convToOutputCharset($conf->global->MAIN_INFO_SOCIETE_NOM)),0,1);

					$pdf->SetFont('','B',6); // fixe la police, le type ( 'B' pour gras, 'I' pour italique, '' pour normal,...)

					$pdf->writeHTMLCell(190,4, 71, 84, dol_htmlentitiesbr($outputlangs->convToOutputCharset($Forme_Juridique_Societe)),0,1);

					$pdf->SetFont('','B',8); // fixe la police, le type ( 'B' pour gras, 'I' pour italique, '' pour normal,...)

					$capital = str_replace('€', '', $conf->global->MAIN_INFO_CAPITAL);

					$pdf->writeHTMLCell(190,4, 140, 83, dol_htmlentitiesbr($outputlangs->convToOutputCharset(price($capital))),0,1);

					$pdf->writeHTMLCell(190,4, 71, 88.5, dol_htmlentitiesbr($outputlangs->convToOutputCharset($Adresse)),0,1);

					$pdf->writeHTMLCell(190,4, 71, 94, dol_htmlentitiesbr($outputlangs->convToOutputCharset($conf->global->MAIN_INFO_SIRET)),0,1);


					$pdf->writeHTMLCell(190,4, 71, 105.2, dol_htmlentitiesbr($outputlangs->convToOutputCharset($conf->global->MAIN_INFO_GERANT)),0,1);

					$pdf->writeHTMLCell(190,4, 71, 110.6, dol_htmlentitiesbr($outputlangs->convToOutputCharset($conf->global->MAIN_INFO_QUALITE_GERANT)),0,1);
			...
			// Page 2
				...
				$pdf->SetFont('','B',10); // fixe la police, le type ( 'B' pour gras, 'I' pour italique, '' pour normal,...)
				$pdf->writeHTMLCell(190,4, 18.5, 119, dol_htmlentitiesbr("X"),0,1);
				...
				if ($details_line->type_mou == '1') {
					...
					$pdf->writeHTMLCell(190,1, 25, 225, dol_htmlentitiesbr(" Entre le 15 et le 20 du mois. Toute situation reçue au-delà de ce délai sera reportée au mois suivant."),0,1);
			...
			// Page 3
				...
				//Contenu
					...
					if ($object->availability) {
						...
						$pdf->writeHTMLCell(190,4, 150, 79, dol_htmlentitiesbr($lib_availability),0,1);
					}
