 ...
 */
class pdf_INPOSE extends ModelePDFFactures
{
    var $db;
    var $name;
    var $description;
	...
	function __construct($db)
	{
		...
		$this->db = $db;
		$this->name = "Facture LMDB";
		$this->description = $langs->trans('PDFLMDBDescription');
...
/**
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		if ($object->situation_cycle_ref && $object->situation_counter > 1) {
			...
			for ($i = 0; $i < count($prevsits); $i++) {
				...
				$pdf->MultiCell($col2x - $col1x, $tab2_hl, $outputlangs->transnoentities("SituationDeduction") . ' ' . ($i + 1) . ' ('. $prevsits[$i]->ref .')',0, 'L', true);
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				if ($prevsits[$i]->total_ht < 0 ) {
					$pdf->MultiCell($largcol2, $tab2_hl, ' + ' . price(-1*$prevsits[$i]->total_ht), 0, 'R', true);
				}else{
	...
	//----  DELEGATION RETENUE DE GARANTIE COMPTE PRORATA  -----------------------------------------------------------------------------------------------------------


				$retenue_de_garantie = $object->total_ttc * $object->array_options['options_lmdb_retenue_garantie'] / 100 ;
				$compte_prorata = $object->total_ttc * $object->array_options['options_lmdb_compte_prorata'] / 100 ;

				$retenue_de_garantie_ht = $object->total_ht * $object->array_options['options_lmdb_retenue_garantie'] / 100 ;
				$compte_prorata_ht = $object->total_ht * $object->array_options['options_lmdb_compte_prorata'] / 100 ;

				$total_ht_restant = $object->total_ht - $retenue_de_garantie_ht - $compte_prorata_ht ;
				$total_ttc_restant = $object->total_ttc - $total_delegation - $retenue_de_garantie - $compte_prorata ;
				$tva_restante = $total_ttc_restant - $total_ht_restant ;


				// Retenue de Garantie TTC
				...
				if ( $retenue_de_garantie != 0 ) {
					...
					$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities('RetenueGarantieTTCDeduitFacture'), $useborder, 'L', 1);
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				...
				if ( $compte_prorata != 0) {
					...
					$pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities('CompteProrataTTCDeduitFacture'), $useborder, 'L', 1);
					$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				...
				if($compte_prorata != 0 || $retenue_de_garantie != 0)
				{
				    ...
				    $pdf->SetFillColor(255,255,255);
				    $pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				    $pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities('LMDBTotalHTRestantFacture'), 0, 'L', 1);
				    ...
				    $pdf->SetFillColor(248,248,248);
				    $pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				    $pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities('LMDBotalTVARestantFacture'), 0, 'L', 1);
				    ...
				    $pdf->SetFillColor(224,224,224);
				    $pdf->SetXY($col1x, $tab2_top + $tab2_hl * $index);
				    $pdf->MultiCell($col2x-$col1x, $tab2_hl, $outputlangs->transnoentities('LMDBTotalTTCRestantFacture'), 0, 'L', 1);
		...
		if ($conf->delegation->enabled)
		{
			...
			if (sizeof($del_lines) > 0)
			{
				...
				$pdf->SetXY($col2x, $tab2_top + $tab2_hl * $index);
				$pdf->MultiCell($largcol2, $tab2_hl, price($object->total_ttc - $total_delegation, 0, $outputlangs), $useborder, 'R', 1);	
			}
