 ...
 */
class pdf_crabe_btp_inpose extends ModelePDFFactures
{
	/**
	...
	function __construct($db)
	{
		...
		if(!empty($conf->global->INVOICE_USE_SITUATION_RETAINED_WARRANTY) && empty($conf->global->INVOICE_USE_RETAINED_WARRANTY)) {
		...
		$this->db = $db;
		$this->name = "BTP LMDB";
		$this->description = $langs->trans('PDFBTPLMDBDescription');
			...
			if (file_exists($dir))
			{
				...
				if ($notetoshow)
				{
					...
					if($pageposafternote>$pageposbeforenote )
					{
						...
						while ($pagenb < $pageposafternote) {
							...
							$pagenb++;
							if (! empty($tplidx)) $pdf->useTemplate($tplidx);
							if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) $this->_pagehead($pdf, $object, 0, $outputlangs, 0, 0, 2);
							// $this->_pagefoot($pdf,$object,$outputlangs,1);
				...
				for ($lineIndex = 0; $lineIndex < $nblignes; $lineIndex++)
				{
					...
					if(!class_exists('TSubtotal') || !TSubtotal::isModSubtotalLine($object->lines[$i])){
						...
						{
							$printval = price($TInfosLigneSituationPrecedente['total_ht_without_progress'], 0, $outputlangs, 0, 0, 2);
				...
				// Page LCR
				if ($object->mode_reglement_code && $object->mode_reglement_code == 'LCR')
				{
					
					//Chargement du Font
					if (empty($conf->global->MAIN_DISABLE_FPDI))
		            {
		                $pagecount = $pdf->setSourceFile(DOL_DOCUMENT_ROOT.'/custom/delegation/core/modules/facture/doc/lcr/fontlcr.pdf');
		                $tplidx = $pdf->importPage(1);
		            }
					...
					$this->_corpslcr($pdf, $object, 1, $outputlangs);
	...
	protected function _addDelegationSupplierInvoicesSummaryPage(&$pdf, $object, $outputlangs, $tplidx)
	{
		...
		$posy += $line_height;
			...
			$values = array(
				...
				price($invoice->total_ht, 0, $outputlangs, 0, 0, 2),
				price($invoice->total_tva, 0, $outputlangs, 0, 0, 2),
				price($invoice->total_ttc, 0, $outputlangs, 0, 0, 2),
				$invoice_datef,
				$invoice_date_due,
			);
		...
		$totals_values = array(
			price($total_ht, 0, $outputlangs, 0, 0, 2),
			price($total_tva, 0, $outputlangs, 0, 0, 2),
			price($total_ttc, 0, $outputlangs, 0, 0, 2),
			'',
			'',
		);
...
/**
	...
	function _tableau_tot(&$pdf, $object, $deja_regle, $posy, $outputlangs)
	{
		...
		$pdf->MultiCell($labelWidth, $tab2_hl, $outputlangs->transnoentities('DelegationTotalsHeaderAmounts'), 0, 'L', 1);
		$pdf->SetXY($colHtX, $tab2_top + $tab2_hl * $index);
		$pdf->MultiCell($colWidth, $tab2_hl, $outputlangs->transnoentities('DelegationTotalsHeaderHT'), 0, 'R', 1);
		$pdf->SetXY($colTvaX, $tab2_top + $tab2_hl * $index);
		$pdf->MultiCell($colWidth, $tab2_hl, $outputlangs->transnoentities('DelegationTotalsHeaderVAT'), 0, 'R', 1);
		...
		if (! empty($TPreviousIncoice)) {
			$index++;
			$pdf->SetXY($colLabelX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($labelWidth, $tab2_hl, $outputlangs->transnoentities('DelegationPreviousSituationsLabel'), 0, 'L', 1);
			$pdf->SetXY($colHtX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($colWidth, $tab2_hl, price($sign * $prev_total_ht, 0, $outputlangs, 0, 0, 2), 0, 'R', 1);
			$pdf->SetXY($colTvaX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($colWidth, $tab2_hl, price($sign * $prev_total_tva, 0, $outputlangs, 0, 0, 2), 0, 'R', 1);
			$pdf->SetXY($colTtcX, $tab2_top + $tab2_hl * $index);
		...
		if ($retenue_de_garantie_ttc != 0) {
			...
			$pdf->SetXY($colHtX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($colWidth, $tab2_hl, price($sign * -$retenue_de_garantie_ht, 0, $outputlangs, 0, 0, 2), 0, 'R', 1);
			$pdf->SetXY($colTvaX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($colWidth, $tab2_hl, price($sign * -$retenue_de_garantie_tva, 0, $outputlangs, 0, 0, 2), 0, 'R', 1);
			$pdf->SetXY($colTtcX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($colWidth, $tab2_hl, price($sign * -$retenue_de_garantie_ttc, 0, $outputlangs, 0, 0, 2), 0, 'R', 1);
		}
		...
		if ($compte_prorata_ttc != 0) {
			...
			$pdf->SetXY($colHtX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($colWidth, $tab2_hl, price($sign * -$compte_prorata_ht, 0, $outputlangs, 0, 0, 2), 0, 'R', 1);
			$pdf->SetXY($colTvaX, $tab2_top + $tab2_hl * $index);
		...
		if ($mpvalo_total_ttc != 0) {
			...
			$pdf->SetXY($colHtX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($colWidth, $tab2_hl, price($sign * $mpvalo_total_ht, 0, $outputlangs, 0, 0, 2), 0, 'R', 1);
			$pdf->SetXY($colTvaX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($colWidth, $tab2_hl, price($sign * $mpvalo_total_tva, 0, $outputlangs, 0, 0, 2), 0, 'R', 1);
			$pdf->SetXY($colTtcX, $tab2_top + $tab2_hl * $index);
			$pdf->MultiCell($colWidth, $tab2_hl, price($sign * $mpvalo_total_ttc, 0, $outputlangs, 0, 0, 2), 0, 'R', 1);
		}
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs, $showLinkedObject = TRUE)
	{
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetTextColor(0,0,60);
		$pdf->MultiCell($w, 4, $outputlangs->transnoentities("PDFCrabeDelegationTitle", $object->situation_counter), '', 'R');
	...
	function _corpslcr(&$pdf, $object, $showaddress, $outputlangs)

	{
		...
		//Corps de la lettre
				...
				$pdf->writeHTMLCell(190,4, 10, 90, dol_htmlentitiesbr($textlcr),0,1);

		// Remplissage de la LCR

			//INPOSE
				$pdf->writeHTMLCell(190,4, 110, 200,$this->emetteur->name,0,1);
				$pdf->writeHTMLCell(50,10, 110, 205,$this->emetteur->getFullAddress(),0,1);

			//Ville
				$pdf->writeHTMLCell(190,4, 15, 214, dol_htmlentitiesbr($object->thirdparty->town),0,1);

			//Date
				$pdf->writeHTMLCell(190,4, 70, 214, dol_htmlentitiesbr(dol_print_date($object->date,"day",false,$outputlangs)),0,1);

			//Montant TTC Mois pour contrôle
				$pdf->writeHTMLCell(190,4, 15, 225, dol_htmlentitiesbr(price($total_ttc_restant, 0, $outputlangs, 0, 0, 2)),0,1);

			//Date de Création
				$pdf->writeHTMLCell(190,4, 45, 225, dol_htmlentitiesbr(dol_print_date($object->date,"day",false,$outputlangs)),0,1);

			//Date d'Échéance
				$pdf->writeHTMLCell(190,4, 75, 225, dol_htmlentitiesbr(dol_print_date($object->date_lim_reglement,"day",false,$outputlangs,true)),0,1);

			//Montant
				$pdf->writeHTMLCell(190,4, 175, 225, dol_htmlentitiesbr(price($total_ttc_restant, 0, $outputlangs, 0, 0, 2)),0,1);

			//Nom du Tiré
				$pdf->writeHTMLCell(190,4, 100, 250, $object->thirdparty->name,0,1);

			//Adresse du Tiré
				$pdf->writeHTMLCell(40,10, 100, 255, $object->thirdparty->getFullAddress(),0,1);
			...
			$pdf->writeHTMLCell(60,10, 140, 245, $outputlangs->convToOutputCharset($object->thirdparty->bac->bank),0,1);
			$pdf->writeHTMLCell(60,10, 10, 245, $outputlangs->convToOutputCharset($object->thirdparty->bac->code_banque),0,1);
			$pdf->writeHTMLCell(60,10, 29, 245, $outputlangs->convToOutputCharset($object->thirdparty->bac->code_guichet),0,1);
			$pdf->writeHTMLCell(60,10, 45, 245, $outputlangs->convToOutputCharset($object->thirdparty->bac->number),0,1);
			$pdf->writeHTMLCell(60,10, 72, 245, $outputlangs->convToOutputCharset($object->thirdparty->bac->cle_rib),0,1);
	...
	function defineColumnField($object,$outputlangs,$hidedetails=0,$hidedesc=0,$hideref=0){
		...
		*/
		...
		$this->cols['prev_progress_amount'] = array(
	        ...
	        'status' => false,
			'title' => array(
				'textkey' => $langs->transnoentities('BtpAnteCumul')
			),
			'border-left' => true, // add left line separator
		);
	    if($this->situationinvoice && ! empty($this->TDataSituation['date_derniere_situation']))
		...
		$this->cols['progress_amount'] = array(
			...
			'status' => true,
			'title' => array(
				'textkey' => $langs->transnoentities('Total_HT_Cumul')
			),
			'border-left' => true, // add left line separator
		);
		...
		$this->cols['totalexcltax'] = array(
			...
			'status' => true,
			'title' => array(
				'textkey' => $langs->transnoentities('Situ_encours')
			),
			'border-left' => true, // add left line separator
		);
	...
	function _tableauBtp(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs, $hidetop=0, $hidebottom=0, $currency='')
	{
		...
		$row_text_offset = 3;
		$columns_x = array(
			$this->posx_new_cumul + $cell_padding,
			$this->posx_cumul_anterieur + $cell_padding,
			$this->posx_month + $cell_padding
		);

		// EN: Build rows dynamically for a flexible layout.
		// FR: Construire les lignes dynamiquement pour un tableau souple.
		$rows = array();
		$rows[] = array(
			'label' => $outputlangs->transnoentities("BtpAmountsHT"),
			'values' => array(
				$this->TDataSituation['nouveau_cumul'],
				$this->TDataSituation['cumul_anterieur'],
				$this->TDataSituation['mois']
			)
		);

		$vat_rates = array();
		if (! empty($this->TDataSituation['vat_rates']) && is_array($this->TDataSituation['vat_rates'])) {
			$vat_rates = $this->TDataSituation['vat_rates'];
		}

		if (! empty($vat_rates)) {
			foreach ($vat_rates as $vatRateInfo) {
				$rateLabel = price($vatRateInfo['rate'], 0, $outputlangs, 0, 0, 2).'%';
				$rows[] = array(
					'label' => $outputlangs->transnoentities("BtpAmountsVatRate", $rateLabel),
					'values' => array(
						$vatRateInfo['nouveau_cumul'],
						$vatRateInfo['cumul_anterieur'],
						$vatRateInfo['mois']
					)
				);
			}
			...
			$rows[] = array(
				'label' => $outputlangs->transnoentities("BtpAmountsVat"),
				'values' => array(
					$this->TDataSituation['nouveau_cumul_tva'],
					$this->TDataSituation['cumul_anterieur_tva'],
					$this->TDataSituation['mois_tva']
				)
			);
		}

		$rows[] = array(
			'label' => $outputlangs->transnoentities("BtpAmountsTTC"),
			'values' => array(
				$this->TDataSituation['nouveau_cumul_ttc'],
				$this->TDataSituation['cumul_anterieur_ttc'],
				$this->TDataSituation['mois_ttc']
			),
			'bold' => true,
			'highlight' => true
		);
		...
		$rows[] = array(
			'label' => $outputlangs->transnoentities("BtpRetenueGarantie"),
			'values' => array(
				$this->TDataSituation['retenue_garantie'],
				$this->TDataSituation['retenue_garantie_anterieure'],
				$this->TDataSituation['retenue_garantie_mois']
			)
		);
		$rows[] = array(
			'label' => $outputlangs->transnoentities("DelegationMpValoTotalLabel"),
			'values' => array(
				$this->TDataSituation['mpvalo_nouveau_cumul'],
				$this->TDataSituation['mpvalo_cumul_anterieur'],
				$this->TDataSituation['mpvalo_mois']
			)
		);
		$rows[] = array(
			'label' => $outputlangs->transnoentities("Compte Prorata"),
			'values' => array(
				$this->TDataSituation['compte_prorata'],
				$this->TDataSituation['compte_prorata_anterieur'],
				$this->TDataSituation['compte_prorata_mois']
			)
		);
		$rows[] = array(
			'label' => $outputlangs->transnoentities("DelegationPayments"),
			'values' => array(
				$this->TDataSituation['delegation_paiement'],
				$this->TDataSituation['delegation_paiement_anterieur'],
				$this->TDataSituation['delegation_paiement_mois']
			)
		);
		$rows[] = array(
			'label' => $outputlangs->transnoentities("BtpRayToRest"),
			'values' => array(
				$this->TDataSituation['total_ttc'],
				$this->TDataSituation['total_ttc_anterieur'],
				$this->TDataSituation['total_ttc_mois']
			),
			'bold' => true
		);
		...
		if (empty($hidetop))
		{
			...
			if (! empty($conf->global->MAIN_PDF_TITLE_BACKGROUND_COLOR))
			{
				$pdf->Rect($this->posx_new_cumul-1, $tab_top, $width, $header_height, 'F', null, explode(',',$conf->global->MAIN_PDF_TITLE_BACKGROUND_COLOR));
				if ($highlight_row_index !== null) {
					$pdf->Rect($this->marge_gauche, $tab_top + $header_height + ($row_height * $highlight_row_index), $this->page_largeur-$this->marge_gauche-$this->marge_droite, $row_height, 'F', null, explode(',',$conf->global->MAIN_PDF_TITLE_BACKGROUND_COLOR));
				}
			}
			else
			{
		...
		for ($row_index = 0; $row_index <= $row_count; $row_index++) {
			$line_y = $tab_top + $header_height + ($row_height * $row_index);
			$line_start_x = ($row_index === 0 ? $this->posx_new_cumul-1 : $this->marge_gauche);
			$pdf->line($line_start_x, $line_y, $this->page_largeur-$this->marge_droite, $line_y);
		}
		
		// ADD TEXT INTO CELL
		foreach ($rows as $row_index => $row) {
			$row_y = $tab_top + $header_height + ($row_height * $row_index);
			$is_bold = (! empty($row['bold']));
			$font_size = $default_font_size - ($is_bold ? 1 : 2);

			$pdf->SetFont('', ($is_bold ? 'B' : ''), $font_size);
			$pdf->SetXY($this->marge_gauche+2, $row_y + $row_text_offset);
			$pdf->MultiCell(80, 2, $row['label'], '', 'C');

			foreach ($row['values'] as $col_index => $value) {
				if (! isset($columns_x[$col_index])) {
					continue;
				}
				$pdf->SetXY($columns_x[$col_index], $row_y + $row_text_offset);
				$pdf->MultiCell($column_width - ($cell_padding * 2), 2, price($value, 0, $outputlangs, 0, 0, 2), '', 'R');
			}
		}
	...
	function _getDataSituation(&$object) 
	{
		...
		$vat_rates_sorted = array();
		foreach ($vat_rates_keys as $vat_rate_key) {
			$vat_rates_sorted[$vat_rate_key] = $vat_rates[$vat_rate_key];
		}
		$TDataSituation['vat_rates'] = $vat_rates_sorted;
	...
	public function setNewPage($posy,   &$pdf, &$object, $outputlangs,$maxY = 168)
	{
		...
		if ($posy > $maxY) {
			...
			$pdf->addPage();
			$pdf->setY($this->marge_haute);
			if (empty($conf->global->MAIN_PDF_DONOTREPEAT_HEAD)) $posy = $this->_pagehead($pdf, $object, 0, $outputlangs, 0, 0, 2);
